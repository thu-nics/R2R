...
try:
    ...
    print("torchviz is not installed, cannot visualize the router model")
    make_dot = None
...
def visualize_model(
    ...
    """
    Visualize the model architecture both as text and as a diagram.
    
    Args:
        model: The PyTorch model to visualize
        output_file: Where to save the visual diagram
    """
    
    input_size = getattr(model, "_init_args", {}).get("logits_size", None)
    hidden_size = getattr(model, "_init_args", {}).get("hidden_states_size", None)
    ...
    if isinstance(model, LogitsClassifier):
        print(f"Model Type: LogitsClassifier")
        print(f"Input: Logits (size: {input_size})")
        if hasattr(model, 'normalize_input') and model.normalize_input:
            print(f"Input Normalization: Enabled (Softmax)")
        print(
            f"Logits projection: {input_size} → {model.logits_projection.out_features}"
        )
        current_dim = model.logits_projection.out_features
        
    elif isinstance(model, HiddenStatesClassifier):
        print(f"Model Type: HiddenStatesClassifier")
        print(f"Input: Hidden States (size: {hidden_size})")
        if hasattr(model, 'normalize_input') and model.normalize_input:
            print(f"Input Normalization: Enabled (LayerNorm)")
        print(
            f"Hidden states projection: {hidden_size} → {model.hidden_states_projection.out_features}"
        )
        current_dim = model.hidden_states_projection.out_features
            
    elif isinstance(model, HiddenStatesClassifierWithLMHead):
        print(f"Model Type: HiddenStatesClassifierWithLMHead")
        print(f"Input: Hidden States (size: {hidden_size})")
        if hasattr(model, 'normalize_input') and model.normalize_input:
            print(f"Input Normalization: Enabled (LayerNorm)")
        print(f"LM Head: {hidden_size} → {model.lm_head.out_features}")
        
        if model.topk is not None:
            print(f"Top-k selection: k={model.topk}")
            print(
                f"Post-LM projection: {model.topk} → {model.post_lm_projection.out_features}"
            )
        else:
            print(
                f"Post-LM projection: {model.lm_head.out_features} → {model.post_lm_projection.out_features}"
            )
            
        current_dim = model.post_lm_projection.out_features
    
    elif isinstance(model, MultiLogitsClassifier):
        print(f"Model Type: MultiLogitsClassifier")
        print(f"Input: Multiple Logits (size: {input_size})")
        print(f"Neural Window Size: {model.neural_window_size}")
        if hasattr(model, 'normalize_input') and model.normalize_input:
            print(f"Input Normalization: Enabled (Softmax per token)")
        print(
            f"Logits projection: {input_size} → {model.logits_projection.out_features}"
        )
        current_dim = model.logits_projection.out_features

    elif isinstance(model, MultiHiddenStatesClassifier):
        print(f"Model Type: MultiHiddenStatesClassifier")
        print(f"Input: Multiple Hidden States (size: {hidden_size})")
        print(f"Neural Window Size: {model.neural_window_size}")
        if hasattr(model, 'normalize_input') and model.normalize_input:
            print(f"Input Normalization: Enabled (LayerNorm per token)")
        print(
            f"Hidden states projection: {hidden_size} → {model.hidden_states_projection.out_features}"
        )
        current_dim = model.hidden_states_projection.out_features

    elif isinstance(model, MultiClassLogitsClassifier):
        print(f"Model Type: MultiClassLogitsClassifier")
        print(f"Input: Multiple Logits (size: {input_size})")
        print(f"Neural Window Size: {model.neural_window_size}")
        if hasattr(model, 'normalize_input') and model.normalize_input:
            print(f"Input Normalization: Enabled (Softmax per token)")
    ...
    elif isinstance(model, MultiClassHiddenStatesClassifier):
        print(f"Model Type: MultiClassHiddenStatesClassifier")
        print(f"Input: Multiple Hidden States (size: {hidden_size})")
        print(f"Neural Window Size: {model.neural_window_size}")
        if hasattr(model, 'normalize_input') and model.normalize_input:
            print(f"Input Normalization: Enabled (LayerNorm per token)")
    ...
    elif isinstance(model, MultiClassHiddenStatesClassifierWithLMHead):
        print(f"Model Type: HiddenStatesClassifierWithLMHead")
        print(f"Input: Hidden States (size: {hidden_size})")
        if hasattr(model, 'normalize_input') and model.normalize_input:
            print(f"Input Normalization: Enabled (LayerNorm)")
        ...
        # Generic structure for any model
        print(model)
        return
    ...
    if backbone.use_position_embedding:
        print(
            f"Position Embeddings: Enabled (max length: {backbone.position_embedding.shape[0]})"
        )
    ...
    for i, block in enumerate(backbone.blocks):
        ...
        exp_dim = input_dim * block.mlp[0].out_features // input_dim
        
        print(f"  Layer: LayerNorm({input_dim})")
        
        if input_dim != output_dim:
            print(f"  Dimension change: {input_dim} → {output_dim}")
            
        print(f"  Layer: Linear({input_dim} → {exp_dim})")
        print(f"  Layer: GELU + Dropout({block.mlp[2].p})")
        print(f"  Layer: Linear({exp_dim} → {output_dim}) + Dropout({block.mlp[4].p})")
        print(f"  Residual connection: + Input({output_dim})")
    
    # Output layer details based on model type
    if hasattr(model, 'num_classes') and model.num_classes > 1:
        print("Output layers:")
        print(f"  Layer: LayerNorm({backbone.blocks[-1].output_dim})")
        print(f"  Layer: Linear({backbone.blocks[-1].output_dim} → {model.num_classes})")
        print(f"  Layer: Softmax")
    else:
        print("Output layers:")
        print(f"  Layer: LayerNorm({backbone.blocks[-1].output_dim})")
        print(f"  Layer: Linear({backbone.blocks[-1].output_dim} → 1)")
    
    # Visual diagram using torchviz if available
    try:
        ...
        if isinstance(model, LogitsClassifier):
            logits = torch.randn(1, input_size).to(device).to(dtype)
            y = model(logits)
        elif isinstance(model, HiddenStatesClassifier):
            hidden = torch.randn(1, hidden_size).to(device).to(dtype)
            y = model(hidden)
        elif isinstance(model, HiddenStatesClassifierWithLMHead):
            hidden = torch.randn(1, hidden_size).to(device).to(dtype)
            y = model(hidden)
        ...
        print(f"\nVisual diagram saved to {output_file}")
    except Exception as e:
        print(f"\nFailed to create visual diagram: {e}")
